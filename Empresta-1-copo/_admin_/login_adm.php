...
if(isset($_POST['btn-entrar'])):
	...
	if(empty($login) or empty($senha)):
		...
		$sql = "SELECT Email_admin FROM admin WHERE Email_admin = '$login'";
		$resultado = mysqli_query($connect, $sql);

		if(mysqli_num_rows($resultado) > 0):
		$senha = md5($senha);
		$sql = "SELECT * FROM admin WHERE Email_admin = '$login' AND Senha_admin = '$senha'";
		$resultado = mysqli_query($connect, $sql);
			...
			if(mysqli_num_rows($resultado) == 1):
				...
				$_SESSION['idadmin'] = $dados['idadmin'];
				header('Location: /_admin_/admin_index.php');
			else:
...
<body class="" style="background-color:#5F9EA0;">
    ...
    <div class="container">
        <div class="card card-login mx-auto mt-5">
            <div class="card-header">Empresta um Copo - Administrador </div>
            <div class="card-body">
                <?php
if(!empty($erros)):
